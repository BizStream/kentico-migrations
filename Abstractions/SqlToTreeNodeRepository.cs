    ...
    where TNode : TreeNode, new()
{
    ...
    /// <summary>
    /// Inserts the passed-in <see cref="TreeNode"/> into the content tree, with overlapping fields mapped from the model.
    /// </summary>
    ...
    public virtual void Insert(TModel model)
    {
        ...
        {
            WriteLine($"Parent not found for {model.NodeName}");
        }
    }


    /// <summary>
    /// Inserts a list of <see cref="TreeNode"/>s into the content tree, with overlapping fields mapped from the model.
    /// </summary>
    /// <param name="models">The list of models from which the inserted node pulls its properties.</param>
    public virtual void InsertAll(IEnumerable<TModel> models)
    {
